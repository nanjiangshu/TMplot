...
#   added a flag to output the identical matrix
# ChangeLog 2017-05-11
#   added a flag so that the unnecessary gaps can be kept
...
Options:
  ...
  -widtmatrix FILE  Output the identical matrix in json format
...
see documentation for details
  ...
  -keep-gaps     Whether keep unnecessary gaps (default: false)
...
#   topoSeqList  Untrimmed topology sequences
    ...
    if not g_params['isKeepUncessaryGap']:
...
#}}}
def WriteSortedClusteredTopoMSA(clusterFile, consensusFile, #{{{
        idList, topoSeqList, posTMList, Mcmp):
    numSeq = len(idList)
# ChangeLog 2015-11-27
# Write also the consensus topology file
...
# get consensus topo of each cluster
    ...
    for i in xrange(len(clusterList)):
        ...
        if fpout:
            consPostTM = myfunc.GetTMPosition(consensusTopo)
            fpout.write(">cons%d numTM=%d numSeqInCluster=%d %s\n"%(i+1, 
                len(consPostTM), len(clusterList[i]), " ".join(idtSeqIDList)))
            fpout.write("%s\n"%(consensusTopo))
    if fpout != None:
        if g_params['verbose'] > 0:
            sys.stdout.write(("Consensus topology file " +
                "output to %s\n" % (consensusFile)))
        fpout.close()

# write pairwise topoaln file
    if os.path.exists(consensusFile):
        outpath = os.path.dirname(clusterFile)
        pairwisetopoalnfile = "%s/%s.cons_pairaln.topo.fa"%(outpath, g_params['rootname'])
        try:
        ...
        for i in xrange(numCluster):
            for j in xrange(i+1, numCluster):
                ...
                if numSeqInClusterList[pair[0]] <= 1 or numSeqInClusterList[pair[1]] <= 1:
                    continue
                if fpout:
                    anno1 = "%s_%d"%(consIDList[pair[0]], numSeqInClusterList[pair[0]])
                    anno2 = "%s_%d"%(consIDList[pair[1]], numSeqInClusterList[pair[1]])
                    pairSeqList = []
                    pairSeqList.append(consTopoList[pair[0]])
                    pairSeqList.append(consTopoList[pair[1]])
                    if not g_params['isKeepUncessaryGap']:
                        lcmp.RemoveUnnecessaryGap(pairSeqList)
                    fpout.write(">%s\n"%(anno1))
                    fpout.write("%s\n"%(pairSeqList[0]))
                    fpout.write(">%s\n"%(anno2))
                    fpout.write("%s\n"%(pairSeqList[1]))
        ...
        if fpout != None:
            if g_params['verbose'] > 0:
                sys.stdout.write(("Pairwise topology file " +
                    "output to %s\n" % (pairwisetopoalnfile)))
            fpout.close()
...
def MultipleTopologyComparison_numTM(topoRecordList, g_params):#{{{
    ...
    outConsensusClusterTopoFile = "%s/%s.cons_of_cluster.m%d.topomsa.fa"%(outpath,
            rootname, g_params['method_topology_comparison'])     
    WriteSortedClusteredTopoMSA(outSortedClusteredTopoMSAFile,
            outConsensusClusterTopoFile, idList, topoSeqList, posTMList, Mcmp)
...
# Nterm topology. 2011-09-22
    ...
    if g_params['outSortedClusteredTopoMSAFile'] != "":
        outpath_clusterfile = os.path.dirname(g_params['outSortedClusteredTopoMSAFile'])
        outConsensusClusterTopoFile = "%s/%s.cons_of_cluster.topomsa.fa"%(outpath_clusterfile, g_params['rootname'])
...
def main(g_params):#{{{
    ...
    while i < numArgv:
        if isNonOptionArg == True:
            ...
            elif (argv[i] in ["-widtmatrix","--widtmatrix"]):
                g_params['outIdenticalTopFile'], i=myfunc.my_getopt_str(argv, i)
            ...
            elif (argv[i] in ["-keep-gaps", "--keep-gaps"]):
                g_params['isKeepUncessaryGap'] = True; i += 1
            elif argv[i] == "-wohtml":
...
def InitGlobalParameter():#{{{
    ...
    g_params['isKeepUncessaryGap'] = False
