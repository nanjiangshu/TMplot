...
def GetNtermState(topo):#{{{
    # updated 2016-09-13
    #   works also with topologies with signal peptide
    # updated 2016-10-11
    #   solving the problem with truncted topology where the starting state
    #   is M
    iostate =  "io"

    tmptopo = topo.replace('S', GAP)
    if tmptopo[0] == GAP:
        tmptopo=tmptopo.lstrip(GAP)
...
#}}}
def GetTopoStateFraction_withSP(topoSeqList):#{{{
    """return (cnt_i, cnt_o, cntM, cnt_SP, cnt_GAP, per_i, per_o, per_M, per_SP, per_GAP)"""
    lengthAlignment=len(topoSeqList[0])
    numSeq = len(topoSeqList)
    cnt_i = [0]*lengthAlignment
    cnt_o = [0]*lengthAlignment
